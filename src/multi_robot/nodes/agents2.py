...
class Agent(object):
    ...
    def __init__(self,agent_name,action_size,state_size,number_episode,rank,id_forzado,type_robot, min_action_time=0.25):
        self.rank                       = rank
        #self.num_laseres                = 0            #CAMBIO
        self.action_done                = 1
        self.action_done_past           = 1
        self.scan_data_past             = 24*[0]
        #self.cont_act                   = 0
        self.conta                       = 0                         #CAMBIO este esta dos veces, aqui y en la 74
        self.vel_lineal                 = 0
        self.vel_ang                    = 0
    ...
    def scan_data(self):
        ...
        # return scan_data
            ...
            if self.__step_cache_1 == self.step and not self.force_update_1:
                ...
                if sum(self.scan_data_past) != 0:#and self.action_done == self.action_done_past:
                    ...
                    if self.action_done_past != self.action_done and self.action_done_past == 2:     #cuando proviene de una accion 2
                        self.conta = self.vectors.reset_cont(20-val_a_cambiar-self.type_robot)
                        scan_data = self.vectors.act_diff_eq_2(scan_data, self.scan_data_past, val_a_cambiar)

                    elif self.action_done < 2 :
                        self.conta = self.vectors.reset_cont(20-val_a_cambiar-self.type_robot)
                        scan_data = self.vectors.act_diff_inf_2(scan_data, self.scan_data_past, action_done, action_done_past, self.conta, val_a_cambiar)

                    elif self.action_done == 2:
                        self.conta = self.reset_cont(10-val_a_cambiar)
                        scan_data = self.vectors.act_eq_2(scan_data, self.scan_data_past, action_done, action_done_past, self.conta, val_a_cambiar)
                    
                    elif self.action_done > 2 :
                        self.conta = self.reset_cont(20-val_a_cambiar-self.type_robot)
                        scan_data = self.vectors.act_dif_sup_2(self.scan_data_past)
