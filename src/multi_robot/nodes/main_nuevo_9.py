...
if rank>(number_rooms*2)-1:
    ...
    #    para indicar donde empieza el robot, si en la room de la izq o derecha, principalmente cuando solo hay un robot y para indicar la
    if number_rooms == 2:
        if rank == 4:
            rank_cloud = 0
        if rank == 5:
            rank_cloud = 1
        room_ID = rank-(number_rooms*2)
    elif number_rooms == 1 and room_chosen == 0:
        rank_cloud = 0
        room_ID = room_chosen
    elif number_rooms == 1 and room_chosen == 1:
        rank_cloud = 1
        room_ID = room_chosen
    ...
    # Instances
    agents  = Agent("agent"+str(robot_ID),action_size,state_size,number_episode,robot_ID,number_rooms,number_robot,log,room_ID,type_robot,rank_cloud)
    ...
    room_ID_c2 = agents.ID+number_rooms
    ...
    comm.send(rank, dest=room_ID_c2, tag=51)
...
###################################
                ...
                comm.send(state_pa, dest =room_ID_c2, tag = 125)
                log.message("Before receiving action from CLOUD 2, after sending state and calculating action in agent")
                # act_calculated_agent = agents.action
                action_evolve = comm.recv(source=room_ID_c2, tag=100)
...
###################################
                ...
                if agents.done==True or agents.environment.get_goalbox == True:
                    ...
                    agents.done_laser = False
                    agents.done_cam = False
                # sys.stdout.flush()
                ...
                if len(agents.learning.memory_D)>=20:
                    ...
                    if init:
                        ...
                        req=comm.issend(data, dest=room_ID_c1, tag=11)
                ...
                if len(agents.learning.memory_GT)>=2:
                    ...
                    if init_G:
                        ...
                        req_g=comm.issend(data_G, dest=room_ID_c1, tag=21)
                ...
                if agents.old_ID != agents.ID:
                    ...
                    comm.send(rank, dest=(agents.old_ID+number_rooms), tag=91)
                ...
                if agents.time_out(step):
                    ...
                    Done = True
                    comm.isend(Done, dest=room_ID_c1, tag=321)
                    break
                if agents.done:
                    #check if has to go at the begining of done
                    if not comm.Iprobe(source=room_ID_c1,tag=321):
                        Done = True
                        log.message("before Done from " +str(agents.ID)+" to "+str(rank))
                        comm.isend(Done, dest=0, tag=321)
    ...
    with tf.device('/cpu:0'):
        ...
        while not rospy.is_shutdown():
            ...
            if ((step%20==0) and (step>0)) and (train==True):
                ...
                if not comm.Iprobe(source=MPI.ANY_SOURCE,tag=444):
                    weight_q      = cluster.q_model.get_weights()
                    weight_target = cluster.target_model.get_weights()
                    ...
                    comm.send(weight_q,dest=send_c2,tag=14)
...
if rank>number_rooms-1 and rank<number_rooms*2:
    ...
    while not rospy.is_shutdown():
        ...
        for g in Room_member_ID:
            if comm.Iprobe(source=g, tag=125):            # PARA UN UNICO ROBOT EN LA HABITACION
                state_and_pa = comm.recv(source=g, tag = 125)
        ...
        if comm.Iprobe(source=send_c1,tag=14):
            ...
            log.message("12.0 after receive q network from "+ str(send_c1) +" TAG: "+ str(14) +" and "+ str(24))
