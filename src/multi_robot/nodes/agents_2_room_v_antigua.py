    ...
    def scan_data(self):
        ...
        # return scan_data
            ...
            if self.__step_cache == self.step and not self.force_update:
                ...
                #-  -----------------------------------------------------------------------------------------------------------------
                # val_a_cambiar = self.vectors.check_laser(scan_data[48])
                ...
                #         self.vectors.check_dist_angle(self.diff_time, self.vel_lineal, self.vel_ang, self.conta)
                #
                #         if self.action_done_past != self.action_done and self.action_done_past == 2:     #cuando proviene de una accion 2
                #             self.conta = self.vectors.reset_cont(self.action_done, self.action_done_past, self.conta, 20-val_a_cambiar-self.type_robot)
                #             scan_data = self.vectors.act_diff_eq_2(scan_data, self.scan_data_past, val_a_cambiar)
                #
                #         elif self.action_done < 2 :
                #             self.conta = self.vectors.reset_cont(self.action_done, self.action_done_past, self.conta, 20-val_a_cambiar-self.type_robot)
                #             scan_data = self.vectors.act_diff_inf_2(scan_data, self.scan_data_past, self.action_done, self.action_done_past, self.conta, val_a_cambiar)
                #
                #         elif self.action_done == 2:
                #             self.conta = self.vectors.reset_cont(self.action_done, self.action_done_past, self.conta, 10-val_a_cambiar)
                #             scan_data = self.vectors.act_eq_2(scan_data, self.scan_data_past, self.action_done, self.action_done_past, self.conta, val_a_cambiar)
                #
                #         elif self.action_done > 2 :
                #             self.conta = self.vectors.reset_cont(self.action_done, self.action_done_past, self.conta, 20-val_a_cambiar-self.type_robot)
                #             scan_data = self.vectors.act_diff_sup_2(scan_data, self.scan_data_past, self.action_done, self.action_done_past, self.conta, val_a_cambiar)
                #
                #         elif self.action_done == 5 and self.action_done_past == 5:
                #             self.conta = 0
                #             scan_data = self.vectors.act_5(scan_data, self.scan_data_past)
                #         else:
                #             self.conta = 0
                #
