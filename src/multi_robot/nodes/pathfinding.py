...
class pathfinding(object):
    ...
    def __init__(self,laser_angles,x_min=-12,x_max=12,y_min=-12,y_max=12,size_s=0.075,debug=False):
        """
          Constructor
        """
        # Map stuff
        ...
        if debug:
            ...
            self.__debug_plt, = plt.plot([0,0],[0,0],zorder=30)
    ...
    def update_map(self,robot_position,target_position,heading,lasers):
        ...
        self.lis_laser = []
        # self.box_tmp   = np.zeros([len(self.x_coordinates),len(self.y_coordinates)])
        self.box_tmp[self.box_tmp>0]   = self.box_tmp[self.box_tmp>0]-1
        ...
        for i,j in enumerate(l_d):
            ...
            if j !=self.max_laser_range:
                self.box_tmp[mask_x,mask_y]+=8    # WALL
    ...
    def monitor(self,robot_pos,target_pos):
        ...
        if self.__debug:
            try:
                ...
                # print("sh",p.shape)
                self.__debug_plt.set_ydata(self.path_cords[:,1])
                self.__debug_plt.set_xdata(self.path_cords[:,0])
