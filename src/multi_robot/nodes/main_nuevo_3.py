...
###################################
                ...
                # weight.saving_weights("agent",agents.step,agents.action,agents.learning.q_model.get_weights(),agents.last_state)
                #
                # if (contador%200==0) and (agents.step>0):
                #     agents.learning.save_model_to_check(rank,contador)
                ...
                action_cluster2 = action_evolve[0]
                evolve_cluster2 = action_evolve[1]
                log.message("action by cloud is: "+str(action_evolve))
                log.message("action calculated by agent network: "+str(agents.action)+"  action calculated by CLOUD 2 network: "+str(action_cluster2))
        ...
        while not rospy.is_shutdown():
            ...
            if ((step%20==0) and (step>0)) and (train==True):
                for g in Room_member_ID:
                    if not comm.Iprobe(source=MPI.ANY_SOURCE,tag=1*(size+1)+rank*10000+g+100):
                        weight_q      = cluster.q_model.get_weights()
                        weight_target = cluster.target_model.get_weights()
                        log.message("11.0 waiting for send network " + str(rank)+ " to "+str(g))
                        #Sending q network to each agent
                        comm.send(weight_q,dest=g,tag=444)
                        ...
                        #Sending target network to each agent
                        comm.send(weight_target,dest=g,tag=455)
                        log.message("11.0  send  target network " + str(rank)+ " to "+str(g))
...
if rank>number_rooms-1 and rank<number_rooms*2:
    ...
    while not rospy.is_shutdown():
        ...
        if comm.Iprobe(source=2, tag=125):            # PARA UN UNICO ROBOT EN LA HABITACION
            state_and_pa = comm.recv(source=2, tag = 125)
