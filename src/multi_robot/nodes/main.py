...
###################################
                ...
                if agents.old_ID != agents.ID:
                    ...
                    comm.send(rank, dest=agents.old_ID, tag=1*(size+1)+agents.old_ID*10000)
                    log.message("tag unsubscribe : "+str(1*(size+1)+agents.old_ID*10000))
            ...
            if ((step%20==0) and (step>0)) and (train==True):
                for g in Room_member_ID:
                    if not comm.Iprobe(source=MPI.ANY_SOURCE,tag=1*(size+1)+rank*10000+g+100):
                        ...
                        comm.send(Room_member_ID,dest=g,tag=3*(size+1)+rank*10000+g+100)
