...
if rank>(number_rooms*2)-1:
    ...
    comm.send(rank, dest=(agents.ID+number_rooms), tag=51)
...
###################################
                ...
                comm.send(state_pa, dest =(agents.ID+number_rooms), tag = 125)
                ...
                action_evolve = comm.recv(source=(agents.ID+number_rooms), tag=100)
...
###################################
                ...
                if agents.old_ID != agents.ID:
                    # room_ID_c1 = agents.ID
                    # room_ID_c2 = room_ID_c1+number_rooms
                    # old_room_ID_c2 = (agents.old_ID+number_rooms)
                ...
                if agents.time_out(step):
                    ...
                    Done = True
                    comm.isend(Done, dest=agents.ID, tag=321)
                    break
                if agents.done:
                    #check if has to go at the begining of done
                    if not comm.Iprobe(source=agents.ID,tag=321):
                        Done = True
    ...
    with tf.device('/cpu:0'):
        ...
        while not rospy.is_shutdown():
            ...
            if ((step%20==0) and (step>0)) and (train==True):
                ...
                if not comm.Iprobe(source=(rank+number_rooms),tag=14):
                    ...
                    comm.send(weight_q,dest=(rank+number_rooms),tag=14)
...
if rank>number_rooms-1 and rank<number_rooms*2:
    ...
    while not rospy.is_shutdown():
        ...
        if comm.Iprobe(source=(rank-number_rooms),tag=14):
            log.message("12.0 before receive network tag:"+str(14))
            weight_q=comm.recv(source=(rank-number_rooms),tag=14)
            ...
            log.message("12.0 after receive q network from "+ str((rank-number_rooms)) +" TAG: "+ str(14) +" and "+ str(24))
