...
class Robot(object):
    ...
    def __init__(self, number_action, environment, min_action_time=0.25):
        ...
        # Save the data laser/camera to avoid asking ros all the time
        self.__step_cache               = -1
        self.step                       = 0
        # Save the data laser to avoid asking ros all the time (used for checking the collision when it is used only one camera)
        self.__step_cache_laser         = -1
        self.step_laser                 = 0
        ...
        # Used for checking the collision when it is used only one camera
        self.force_update_laser               = False
    ...
    def check_laser(self):
        ...
        '''
        if self.__step_cache_laser == self.step and not self.force_update_laser:
            check_laser=self.__scan_data_cache_laser
        else:
            d_laser = None
            ...
            scan_laser = d_laser
            check_laser = []
            # for i in range(len(scan_laser.ranges)):
            #     if scan_laser.ranges[i] == float("Inf"):
            #         check_laser.append(3.5)
            #     elif np.isnan(scan_laser.ranges[i]):
            #         check_laser.append(0)
            #     else:
            #         check_laser.append(scan_laser.ranges[i])

            for i in range(len(scan_laser.ranges)/2):             
                    check_laser.append(scan_laser.ranges[i+24])       #de los 48 valores de scan_laser, los 24 últimos son los correspondientes al LASER

            if np.any(np.isnan(np.array(check_laser))):
                raise Exception("it's nan sensor")

            self.__step_cache_laser      = self.step
            self.__scan_data_cache_laser = check_laser
            self.force_update_laser      = False
        return check_laser
    ...
    def scan_data(self):
        ...
        elif self.laser_scan == True:
            '''
            Get data of the laser         #para una camara
            '''
        ...
        #             scan_data.append(scan.ranges[i])       #de los 48 valores de scan_laser, los 24 primeros son los correspondientes a la CAMARA
    ...
    def state(self):
        ...
        #     self.__coll_count = 0
        #     print("collision with the LASER")
        # else:
        ...
        if ((self.min_range >= min(scan_data) > 0) and (not self.__crashed)) \
            ...
            self.__process = "collision"
            self.__coll_count = 0
            print("collision with the CAMERA")
